...
#define buzzerPin 7 // indicate press event
...
int32_t accelCount[3];  // Stores the 16-bit signed accelerometer sensor output
int32_t gyroCount[3];   // Stores the 16-bit signed gyro sensor output
...
{  
  ...
  if (readByte(MPU9250_ADDRESS, INT_STATUS) & 0x01) {  
    ...
    getMres();
   //magBias[0] = 286.66;  // User environmental x-axis correction in milliGauss, should be automatically calculated//470////////////////////////////////////////////////
   //magBias[1] = 185.81;  // User environmental x-axis correction in milliGauss//120////////////////////////////////////////////////////////////////////////////////////
   //magBias[2] = -347.33;  // User environmental x-axis correction in milliGauss//125////////////////////////////////////////////////////////////////////////////////////
...
//    Serial.print(" qz = "); Serial.println(q[3]); 
  ...
  // For more see http://en.wikipedia.org/wiki/Conversion_between_quaternions_and_Euler_angles which has additional links.
    yaw   = atan2(2.0f * (q[2] * q[1] + q[0] * q[3]), q[0] * q[0] + q[1] * q[1] - q[2] * q[2] - q[3] * q[3]); //120300112233 
    pitch = -asin(2.0f * (q[1] * q[3] - q[0] * q[2]));
    roll  = atan2(2.0f * (q[0] * q[1] + q[2] * q[3]), q[0] * q[0] - q[1] * q[1] - q[2] * q[2] + q[3] * q[3]); //012300112233
    pitch *= 180.0f / PI;
    yaw   *= 180.0f / PI; 
    yaw   += 3.86; /* Declination at Taiwan  Model Used:       IGRF12  Help
                                                               Latitude:  23 N
                                                               Longitude: 120 E
                                                               Date Declination
                                                               2018-05-04 3.86度 W  changing by  0.08度 W per year (+ve for west )*/
    roll  *= 180.0f / PI;
     
    if (BTSerial.available()>0) {
    char inChar = BTSerial.read();
    if (inChar == 'r') {
      BTSerial.print(ax,2); BTSerial.print(",");//unit:g
      BTSerial.print(ay,2); BTSerial.print(",");
      BTSerial.print(az,2); BTSerial.print(",");
      BTSerial.print(yaw+180,2); BTSerial.print(",");
      BTSerial.print(pitch,2); BTSerial.print(",");
      BTSerial.println(roll,2); 
...
if(state == 2 && array_index < 100){
  if (lastWrite + duration < millis()) {
      ...
      // decide state
  int16_t sums= abs(ax) + abs(ay) + abs(az);
...
void readAccelData(int32_t * destination)
{
...
void readGyroData(int32_t * destination)
{
...
void magcalMPU9250(float * dest1, float * dest2) 
 ...
 BTSerial.print("Mag Calibration: Wave device in a figure eight until done!");
...
// Get soft iron correction estimate
 ...
 BTSerial.print("Mag Calibration done!");
